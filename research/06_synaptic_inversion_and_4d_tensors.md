...
[`InvertedDense`](../doraneural/layers.py#L1345) introduces a **learnable continuous polarity dial** $\alpha_j$ for each neuron $j$:
...
In a dual-phase system containing destructive interference ($y = \sum x_{\text{pos}} - 2.5 \sum x_{\text{neg}}$), [`InvertedDense`](../doraneural/layers.py#L1345) rapidly organized its internal neurons into excitatory and inverted inhibitory banks, converging to:
- **Test MSE:** `0.00170`
...
[`Tensor4DDense`](../doraneural/layers.py#L1491) preserves the 4D coordinate topology while applying hyper-tensor channel transformations:
...
- Source code: [`doraneural/layers.py`](../doraneural/layers.py#L1345)
- Test suite: [`tests/test_neural_lib.py`](../tests/test_neural_lib.py#L525)
- Demo script: [`examples/explore_4d_and_inverter.py`](../examples/explore_4d_and_inverter.py)
